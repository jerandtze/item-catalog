...
def genreMenuJSON(genre_id):
    ...
    items = session.query(BookItem).filter_by(genre_id=genre_id).all()
...
@app.route('/genre/')
def showGenres():
    genres = session.query(Genre).order_by(asc(Genre.title))
    if 'username' not in login_session:
...
def newGenre():
    if request.method == 'POST':
        ...
        flash('New Genre %s Successfully Created' % newGenre.title)
        session.commit()
...
# Query database and fetch a single Genre for editing
    ...
    if request.method == 'POST':
        if request.form['title']:
            ...
            flash('Genre Successfully Edited %s' % editedGenre.title)
            return redirect(url_for('showGenres'))
    else:
...
def showMenu(genre_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicmenu.html', items=items, genre=genre, creator=creator)
    else:
...
def editBookItem(genre_id, book_id):
    ...
    if request.method == 'POST':
        ...
        if request.form['coverUrl']:
            editedItem.coverUrl = request.form['coverUrl']
        if request.form['price']:
            editedItem.price = request.form['price']
        if request.form['edition']:
            editedItem.edition = request.form['edition']
        session.add(editedItem)
        session.commit()
        flash('Book Item Successfully Edited')
        return redirect(url_for('showMenu', genre_id=genre_id))
    else:
        return render_template('editbookitem.html', genre_id=genre_id, book_id=book_id, item=editedItem)
